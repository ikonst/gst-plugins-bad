...
static void
gst_core_video_meta_free (GstMeta * gmeta, GstBuffer * buf)
{
  GstCoreVideoMeta *meta;

  meta = (GstCoreVideoMeta *) gmeta;
  CVBufferRelease (meta->cvbuf);
}

static gboolean
gst_core_video_meta_transform (GstBuffer * transbuf, GstMeta * gmeta,
    GstBuffer * buffer, GQuark type, gpointer data)
{
  ...
  meta = (GstCoreVideoMeta *) gmeta;
  copy_data = (GstMetaTransformCopy *) data;
    ...
    CVPixelBufferRef pixel_buf, gboolean * has_padding, gboolean map)
{
  ...
  *has_padding = FALSE;
  ...
  if (CVPixelBufferIsPlanar (pixel_buf)) {
    ...
    if (map) {
      gpixbuf = gst_core_video_pixel_buffer_create (pixel_buf);
    }
    ...
    for (i = 0; i < n_planes; i++) {
      ...
      if (map) {
        gst_buffer_append_memory (buf,
            gst_core_video_memory_new_wrapped (gpixbuf, i, size));
      }
    }

    if (map) {
      gst_core_video_pixel_buffer_unref (gpixbuf);
    }
  } else {
    ...
    if (map) {
      ...
      gpixbuf = gst_core_video_pixel_buffer_create (pixel_buf);
      gst_buffer_append_memory (buf,
          gst_core_video_memory_new_wrapped (gpixbuf, GST_CORE_VIDEO_NO_PLANE,
              size));
      gst_core_video_pixel_buffer_unref (gpixbuf);
    }
  }
